/* https://unpkg.com/docsify@4.7.0/lib/docsify.js */
(function () {
/**
...
function corner(data) {
  ...
  return (
    "<a href=\"" + data + "\" class=\"github-corner\" aria-label=\"View source on Github\" target=\"_blank\">" +
    '<svg viewBox="0 0 70 70" aria-hidden="true">' +
    '<path d="M41.6,22.3c-0.8-0.7-1.6-1.5-2.3-2.2c-1.4,1.1-3,2.2-3.4,4.1c-0.6,3.2,2.7,6.4,5.9,5.6   c1.6-0.3,4.1-1,4.6,1.2c-0.2,1.9-2.4,2.3-4,2.5c0.8,0.8,1.6,1.6,2.5,2.3c1.9-0.8,4.1-2.1,4.4-4.4c0.3-3-2.8-5.8-5.7-5.1   c-1.7,0.1-4.2,1.4-5.1-0.8C38.3,23.7,40.2,23,41.6,22.3z" fill="#0b87da" style="transform-origin: 70px 70px;" class="octo-arm"></path>' +
    '<path d="M0,0l24.2,24.2l13-18.8l27.3,27.3l-18.8,13L70,70V0H0z M64.2,24.7c-0.5,1.1-1.6,2-2.9,2.2l-4.6-4.6   c0.4-2.2,2.6-3.6,4.8-3.1c0.1-0.6,0.2-1.3,0.2-1.9c0-1.1-0.2-2.2-0.6-3.2c-0.6,0.2-1.3,0.3-2,0.2c-1.2-0.2-2.3-0.9-2.9-1.9   c-0.6-1-0.7-2.3-0.2-3.4c-1-0.4-2.1-0.6-3.2-0.6c-0.7,0-1.3,0.1-1.9,0.2c0.5,2.2-0.9,4.4-3.1,4.8l-4.6-4.6c0.1-0.7,0.5-1.4,1-1.9   c0.8-0.8,1.9-1.3,3-1.3c1.2,0,2.3,0.6,3,1.6c0.8-0.2,1.7-0.3,2.6-0.3c1.4,0,2.8,0.3,4.1,0.8c0.6-0.6,1.5-1,2.4-1.1   c1.6-0.1,3.2,0.8,3.9,2.2c0.7,1.5,0.4,3.3-0.8,4.4c0.9,2.1,1.1,4.5,0.5,6.7c0.6,0.4,1,1,1.3,1.6C64.6,22.5,64.6,23.7,64.2,24.7z" fill="#0b87da" class="octo-body"></path>' +
    '</svg>' +
    '</a>'
  )
}
...
Compiler.prototype._initRenderer = function _initRenderer () {
  ...
  origin.paragraph = renderer.paragraph = function (text) {
    var result;
    if (/^p&gt;/.test(text)) {
      result = helper('primary alert', text);
    } else if (/^s&gt;/.test(text)) {
      result = helper('secondary alert', text);
    } else if (/^h&gt;/.test(text)) {
      result = helper('help alert', text);
    } else if (/^b&gt;/.test(text)) {
      result = helper('bug alert', text);
    } else if (/^i&gt;/.test(text)) {
      result = helper('info alert', text);
    } else if (/^c&gt;/.test(text)) {
      result = helper('code alert', text);
    } else {
      result = "<p>" + text + "</p>";
    }
    return result
